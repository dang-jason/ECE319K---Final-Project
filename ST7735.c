...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
			//if(image[i] != 0xFFFF)
				ST7735_DrawPixel(j+x, k-h+y+1, image[i]);
...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor){
  ...
  if(y>15) return 0;
  while(*pt){
    ST7735_DrawCharS(x*6, y*10, *pt, textColor, 0x3C58, 1);
    pt++;
    x = x+1;
...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawCharS(StX*6,StY*10,ch,ST7735_YELLOW,0x3C58,1);
  StX++;
  return;
}
//********ST7735_OutString*****************
...
void Output_Init(void){
  ...
  ST7735_FillScreen(0x1186);                 // set screen to Fireboy & Watergirl background color
